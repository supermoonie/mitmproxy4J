...
import com.github.supermoonie.proxy.fx.ui.main.MainView;
...
public class DumpHttpRequestIntercept implements RequestIntercept {
    ...
    public FullHttpResponse onRequest(InterceptContext ctx, HttpRequest request) {
        try {
            final Request req = RequestDao.saveRequest(ctx, request);
            log.info("request saved, uri: {}", ctx.getConnectionInfo().getUrl());
            ctx.setUserData(req);
            Platform.runLater(() -> {
                try {
                    MainView mainView = App.getMainController();
                    mainView.addFlow(ctx.getConnectionInfo(), req, null);
                } catch (Exception e) {
