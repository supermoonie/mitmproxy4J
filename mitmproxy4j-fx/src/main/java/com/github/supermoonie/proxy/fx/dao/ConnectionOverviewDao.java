 ...
 * @since 2020/11/23
 */
public final class ConnectionOverviewDao {

    private ConnectionOverviewDao() {
    ...
    public static void updateServerInfo(ConnectionInfo connectionInfo, int requestId) throws SQLException {
        ...
        if (null != connectionOverviews && connectionOverviews.size() > 0) {
            ...
            if (null != connectionInfo.getRemoteAddressList()) {
                connectionOverview.setRemoteIpList(JacksonUtil.toJsonString(connectionInfo.getRemoteAddressList().stream().map(InetAddress::getHostAddress).collect(Collectors.toList())));
            }
