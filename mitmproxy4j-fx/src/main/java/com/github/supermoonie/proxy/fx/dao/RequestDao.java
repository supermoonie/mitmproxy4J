    ...
    public static Request saveRequest(InterceptContext ctx, HttpRequest httpRequest) throws SQLException {
        ...
        return transactionManager.callInTransaction(() -> {
            if (httpRequest instanceof FullHttpRequest) {
                int contentId = ContentDao.saveContent(((FullHttpRequest) httpRequest).content());
                req.setContentId(contentId);
            }
            Dao<Request, Integer> dao = DaoCollections.getDao(Request.class);
            dao.create(req);
            ConnectionOverviewDao.saveClientInfo(connectionInfo, req.getId());
            CertificateInfoDao.saveList(connectionInfo.getLocalCertificates(), req.getId(), null);
            HeaderDao.saveHeaders(httpRequest.headers(), req.getId(), null);
            return req;
        });
    }
}
